...
## Simulation study
This code is what has been used to perform the reported simulation study in the masters thesis. NB: The code can take very long to run. It is recommended to only run it if you really need the results. See usage example for an easier example of how to apply the BVI method. 
...
simulate_data <- function(n = 10000, n_groups = 100, covariance_level=0) {
  ...
  random_effect_contributions_z1_pois <- rnorm(n_groups, mean = 0, sd = sqrt(1/7))[Z1]
  ...
  beta1_pois <- 1/sqrt(7)
  beta2_pois <- sqrt(2)/sqrt(7)
  beta3_pois <- sqrt(3)/sqrt(7)
  ...
  eta_pois <- beta1_pois*X1 + beta2_pois*X2 + beta3_pois*X3 + random_effect_contributions_z1_pois
  lambda <- exp(eta_pois)
...
run_simulation <- function(n_sim = 5, covariance_level = 0, include_probit = FALSE) {
  ...
  for (i in 1:n_sim) {
    ...
    importances[[i]] <- list(
      logit = BayesianVariableImportance::extract_importances(model_logit, datasets$binomial_logit, pi^2 / 3),
      poisson = BayesianVariableImportance::extract_importances(model_pois, datasets$poisson, log(1 + 1/exp(summary(model_pois)$fixed[1] + 0.5)))
    )
    ...
    if (include_probit) {
      ...
      importances[[i]]$probit = BayesianVariableImportance::extract_importances(model_probit, datasets$binomial_probit, 1)
    }
...
saveRDS(sim_no, "simulation_no_corr_XX.XX.rds")
saveRDS(sim_low_pos, "simulation_low_pos_corr_XX.XX.rds")
saveRDS(sim_low_neg, "simulation_low_neg_corr_XX.XX.rds")
saveRDS(sim_high_pos, "simulation_high_pos_corr_XX.XX.rds")
saveRDS(sim_high_neg, "simulation_high_neg_corr_XX.XX.rds")
```

```{r}
# To load this file back into R:
setwd("/path/to/your/desired/folder")
loaded_sim_no <- readRDS("simulation_no_corr_XX.XX.rds")
loaded_sim_low_pos <- readRDS("simulation_low_pos_corr_XX.XX.rds")
loaded_sim_low_neg <- readRDS("simulation_low_neg_corr_XX.XX.rds")
loaded_sim_high_pos <- readRDS("simulation_high_pos_corr_XX.XX.rds")
loaded_sim_high_neg <- readRDS("simulation_high_neg_corr_XX.XX.rds")
...
```{r}
setwd("/path/to/your/desired/folder")
saveRDS(rptR_logit_no, "stoffel_sim_logit_no_corr_XX.XX.rds")
saveRDS(rptR_pois_no, "stoffel_sim_pois_no_corr_XX.XX.rds")

saveRDS(rptR_logit_low_pos, "stoffel_sim_logit_low_pos_corr_XX.XX.rds")
saveRDS(rptR_pois_low_pos, "stoffel_sim_pois_low_pos_corr_XX.XX.rds")

saveRDS(rptR_logit_low_neg, "stoffel_sim_logit_low_neg_corr_XX.XX.rds")
saveRDS(rptR_pois_low_neg, "stoffel_sim_pois_low_neg_corr_XX.XX.rds")

saveRDS(rptR_logit_high_pos, "stoffel_sim_logit_high_pos_corr_XX.XX.rds")
saveRDS(rptR_pois_high_pos, "stoffel_sim_pois_high_pos_corr_XX.XX.rds")

saveRDS(rptR_logit_high_neg, "stoffel_sim_logit_high_neg_corr_XX.XX.rds")
saveRDS(rptR_pois_high_neg, "stoffel_sim_pois_high_neg_corr_XX.XX.rds")
```

```{r}
setwd("/path/to/your/desired/folder")
rptR_logit_no <- readRDS("stoffel_sim_logit_no_corr_XX.XX.rds")
rptR_pois_no <- readRDS("stoffel_sim_pois_no_corr_XX.XX.rds")

rptR_logit_low_pos <- readRDS("stoffel_sim_logit_low_pos_corr_XX.XX.rds")
rptR_pois_low_pos <- readRDS("stoffel_sim_pois_low_pos_corr_XX.XX.rds")

rptR_logit_low_neg <- readRDS("stoffel_sim_logit_low_neg_corr_XX.XX.rds")
rptR_pois_low_neg <- readRDS("stoffel_sim_pois_low_neg_corr_XX.XX.rds")

rptR_logit_high_pos <- readRDS("stoffel_sim_logit_high_pos_corr_XX.XX.rds")
rptR_pois_high_pos <- readRDS("stoffel_sim_pois_high_pos_corr_XX.XX.rds")

rptR_logit_high_neg <- readRDS("stoffel_sim_logit_high_neg_corr_XX.XX.rds")
rptR_pois_high_neg <- readRDS("stoffel_sim_pois_high_neg_corr_XX.XX.rds")
```
...
# Combine all successful datasets into one data frame for Logit and Poisson
# Here, using bind_rows from dplyr to merge data frames with an identifier
...
loaded_sim_no$successes[[1]]$logit$expected_importance

expected_importance <- 0.0972#loaded_sim_no$successes[[1]]$logit$expected_importance
...
random_importances_logit$ConditionalExpected <- ifelse(random_importances_logit$Correlation == "0",
                                                      ...
                                                      NA)
# Convert Correlation to a factor and specify the order of the levels
...
medians <- random_importances_logit %>%
  group_by(Correlation) %>%
  summarise(Median = median(RandomImportance))
...
random_logit_plot <- ggplot(random_importances_logit, aes(x = RandomImportance, fill = Correlation)) +
  ...
  #geom_point(data = medians, aes(x = Median, y = 0, fill = Correlation), shape = 21, size = 3, color = "grey") +
  ...
  labs(#title = "Random Importances Across Correlation Levels for Logit Model",
       x = "Relative Importance", y = "Frequency") +
...
expected_importance <- 0.0969 #loaded_sim_no$successes[[1]]$poisson$expected_importance
...
medians <- random_importances_poisson %>%
  group_by(Correlation) %>%
  summarise(Median = median(RandomImportance))
...
plot_fixed_importances <- function(data, model_name, cov_level=0) {
  ...
  for (i in 1:3) {
    expected <- ifelse(model_name == "logit", 0.0972, 0.0969)
    fixed_importances <- data.frame(
      ...
      ExpectedImportance = (i * expected), #data[[1]][[model_name]]$expected_importance),
  ...
  all_fixed_importances$Importance <- round(all_fixed_importances$Importance, 5)
  ...
  medians <- all_fixed_importances %>%
  group_by(FixedEffect) %>%
  summarise(Median = median(Importance))
  
  importance_range <- range(all_fixed_importances$Importance)
  binwidth <- (importance_range[2] - importance_range[1]) / 90
  
  print(binwidth)
  print(importance_range)
  
  if (cov_level == 0) {
    ggplot(all_fixed_importances, aes(x = Importance)) +
    geom_histogram(binwidth = binwidth, fill = fill_color, alpha = 0.7) +
    ...
    geom_point(data = means, aes(x = Mean, y = 0, fill = FixedEffect), shape = 21, size = 3, show.legend = F) +
    #geom_point(data = medians, aes(x = Median, y = 0, fill = FixedEffect), shape = 21, size = 3, color = "grey", show.legend = F) +  
    geom_text(data = means, aes(x = Mean, y = 0, label = FixedEffect), vjust = -1, color = "black") + 
    labs(#title = paste(title_prefix, "Fixed Effect Importances with Expected Importances"),
         #x = "Fixed Effect Importance", 
         y = "Frequency") +
    annotate("text", x = Inf, y = Inf, label = TeX(paste0("$\\rho = $", cov_level)), vjust = 1.5, hjust = 1.2, size = 7, color = "#696969") + 
...
poisson_no <- remove_x_axis_label(plot_fixed_importances(success_no_pois, "poisson", cov_level = 0))
poisson_low_pos <- remove_x_axis_label(plot_fixed_importances(success_low_pos_pois, "poisson", cov_level = 0.1))
poisson_low_neg <- remove_x_axis_label(plot_fixed_importances(success_low_neg_pois, "poisson", cov_level = -0.1))
poisson_high_pos <- remove_x_axis_label(plot_fixed_importances(success_high_pos_pois, "poisson", cov_level = 0.4))
poisson_high_neg <- remove_x_axis_label(plot_fixed_importances(success_high_neg_pois, "poisson", cov_level = -0.4))

combined_plot_logit <- (logit_high_neg / logit_low_neg / logit_no / logit_low_pos / logit_high_pos) +
  plot_layout(ncol = 1, axes = "collect", guides = "auto") +
  plot_annotation(title = NULL, caption = NULL, tag_levels = NULL) 


combined_plot_logit <- combined_plot_logit + theme_minimal() + theme(text = element_text(family="LM Roman 10"), strip.text.x = element_blank(),
                                       strip.text.y = element_blank(),
                                       axis.title.x = element_text(size = 16),
                                       axis.title.y = element_text(size = 16),
                                       axis.text.x = element_text(size = 12),
                                       axis.text.y = element_text(size = 12)
                                       ) + labs(x = "Relative Importance") + plot_layout(axes = "collect")
...
combined_plot_poisson <- combined_plot_poisson + theme_minimal() + theme(text = element_text(family="LM Roman 10"), strip.text.x = element_blank(),
                                       strip.text.y = element_blank(),
                                       axis.title.x = element_text(size = 16),
                                       axis.title.y = element_text(size = 16),
                                       axis.text.x = element_text(size = 12),
...
for (i in 1:length(rho_values)) {
  ...
  var_eta_poisson <- (sum((beta_poisson^2)) + 
                2*(beta_poisson[1] * beta_poisson[2] * rho
               + beta_poisson[1] * beta_poisson[3] * rho
               + beta_poisson[3] * beta_poisson[2] * rho)
               + 1/7)
  
  lamba_poisson <- exp(0 + 0.5*((sum((beta_poisson^2)) + 
                        2*(beta_poisson[1] * beta_poisson[2] * rho
                        + beta_poisson[1] * beta_poisson[3] * rho
                        + beta_poisson[3] * beta_poisson[2] * rho)
                        + 1/7)))
  
  dist_poisson <- log(1+1/lamba_poisson)
  
  #cat(var_Y)
  if (rho==0){
    variance_explained[i, "expected_logit"] <- 1/(var_eta + dist_logit)
    variance_explained[i, "expected_poisson"] <- (1/7)/(var_eta_poisson + dist_poisson)
  ...
  R2_marginal_poisson <- (var_eta_poisson - 1/7) / (var_eta_poisson + dist_poisson)
  R2_conditional_poisson <- (var_eta_poisson) / (var_eta_poisson + dist_poisson)
...
# Function to plot R-squared values including Stoffel points
plot_r2_correlation_level_with_stoffel <- function(data, model_name, correlation_label, correlation_color, stoffel_values) {
    ...
    # Retrieve the expected values from the variance_explained DataFrame
    ...
    # Prepare Stoffel values as points
    ...
    facet_titles <- c(
        "Marginal" = "Marginal~R^2",
        "Conditional" = "Conditional~R^2"
    )
    # Plot
    if (correlation_label == -0.4){
      plot <- ggplot(r2_data, aes(x = Value, fill = Type)) +
          geom_histogram(bins = 30, alpha = 0.7) +
          facet_wrap(~ Type, scales = "free", labeller = labeller(Type = as_labeller(facet_titles, label_parsed))) +#as_labeller(facet_titles)) +
          ...
          #geom_point(data = medians, aes(x = Median, y = 0, fill = Type), shape = 21, size = 3, color = "grey") +
          geom_point(data = stoffel_points, aes(x = Value, y = 0, color = Measure), size = 3) +
          annotate("text", x = Inf, y = Inf, label = TeX(paste0("$\\rho =$", correlation_label)), vjust = 1.5, hjust = 1.2, size = 7, color = "#696969") +
...
r2_data_high_neg <- data.frame(
    ...
    Value = c(sapply(success_high_neg_bin, function(x) x[["logit"]]$r2m),
              sapply(success_high_neg_bin, function(x) x[["logit"]]$r2c))
)
...
r2_data_low_neg <- data.frame(
    ...
    Value = c(sapply(success_low_neg_bin, function(x) x[["logit"]]$r2m),
              sapply(success_low_neg_bin, function(x) x[["logit"]]$r2c))
)
...
r2_data_no <- data.frame(
    ...
    Value = c(sapply(success_no_bin, function(x) x[["logit"]]$r2m),
              sapply(success_no_bin, function(x) x[["logit"]]$r2c))
)
...
r2_data_low_pos <- data.frame(
    ...
    Value = c(sapply(success_low_pos_bin, function(x) x[["logit"]]$r2m),
              sapply(success_low_pos_bin, function(x) x[["logit"]]$r2c))
)
...
r2_data_high_pos <- data.frame(
    ...
    Value = c(sapply(success_high_pos_bin, function(x) x[["logit"]]$r2m),
              sapply(success_high_pos_bin, function(x) x[["logit"]]$r2c))
)
...
combined_plot_r2_logit <- combined_plot_r2_logit + theme_minimal() + theme(text = element_text(family="LM Roman 10"), legend.position = "none",
                                       strip.text.y = element_blank(),
                                       axis.title.x = element_blank(),
                                       axis.title.y = element_text(size = 16),
                                       axis.text.x = element_text(size = 12),
...
combined_plot_r2_poisson <- combined_plot_r2_poisson + theme_minimal() + theme(text = element_text(family="LM Roman 10"), legend.position = "none",
                                       strip.text.y = element_blank(),
                                       axis.title.x = element_blank(),
                                       axis.title.y = element_text(size = 16),
                                       axis.text.x = element_text(size = 12),
