...
# The full code to produce plots is given below, and the data and sourcecode can be found under the folder "Animal model source code and data" in the repository.
setwd("~/Animal model example/Models and samples")


model_mass <- readRDS("Models and samples/model_mass_10.05.rds")
samples_mass <- readRDS("Models and samples/samples_mass_10.05.rds")

model_wing <- readRDS("Models and samples/model_wing_10.05.rds")
samples_wing <- readRDS("Models and samples/samples_wing_10.05.rds")

model_tarsus <- readRDS("Models and samples/model_tarsus_10.05.rds")
samples_tarsus <- readRDS("Models and samples/samples_tarsus_10.05.rds")
```
...
heritability_mass <- ggplot(samples_mass$scaled_random_samples, aes(x = !!sym(additive_genetic))) +
  ...
  #geom_point(aes(x = mean(silva_mass), y = 0), color = "#FFD700", size = 4) +
  #geom_point(aes(x = mean(muff_mass), y = 0), color = "#32CD32", size = 4) +
  annotate("point", x = mean(samples_mass$scaled_random_samples$IDC2), y = 0, color = "#E6A0C4", size = 4) +
  ...
  geom_vline(aes(xintercept = CI_mass[1]), color = "#E6A0C4", linewidth = 1, linetype = "dashed") +
  geom_vline(aes(xintercept = CI_mass[2]), color = "#E6A0C4", linewidth = 1, linetype = "dashed") +
...
heritability_wing <- ggplot(samples_wing$scaled_random_samples, aes(x = !!sym(additive_genetic))) +
  ...
  #geom_point(aes(x = mean(silva_wing), y = 0), color = "#FFD700", size = 4) +
  #geom_point(aes(x = mean(muff_wing), y = 0), color = "#32CD32", size = 4) +
  annotate("point", x = mean(samples_wing$scaled_random_samples$IDC2), y = 0, color = "#E6A0C4", size = 4) +
  ...
  geom_vline(aes(xintercept = CI_wing[1]), color = "#E6A0C4", linewidth = 1, linetype = "dashed") +
  geom_vline(aes(xintercept = CI_wing[2]), color = "#E6A0C4", linewidth = 1, linetype = "dashed") +
...
heritability_tarsus <- ggplot(samples_tarsus$scaled_random_samples, aes(x = !!sym(additive_genetic))) +
  ...
  #geom_point(aes(x = mean(silva_tarsus), y=0), color = "#FFD700", size = 4) +
  annotate("point", x = mean(samples_tarsus$scaled_random_samples$IDC2), y = 0, color = "#E6A0C4", size = 4) +
  ...
  geom_vline(aes(xintercept = CI_tarsus[1]), color = "#E6A0C4", linewidth = 1, linetype = "dashed") +
  geom_vline(aes(xintercept = CI_tarsus[2]), color = "#E6A0C4", linewidth = 1, linetype = "dashed") +
...
## The below code is made with the data available. A pedigree structure is inialized in the source code, which is not available here. 

```{r, echo = FALSE, eval = FALSE}
# Add your path to the source code file here:
setwd("~/BayesianVariableImportance/Animal model example")
source("../Animal model source code and data/DataPrep.R")
str(d.morph)
...
timing_result_mass <- microbenchmark(
  {
    model_mass <- BayesianVariableImportance::perform_inla_analysis(d.morph, formula.mass, "gaussian", priors = prior_mass)
...
timing_result_wing <- microbenchmark(
  {
    model_wing <- BayesianVariableImportance::perform_inla_analysis(d.morph, formula.wing, "gaussian", priors = prior_wing)
...
timing_result_tarsus <- microbenchmark(
  {
      model_tarsus <- BayesianVariableImportance::perform_inla_analysis(d.morph, formula.tarsus, "gaussian", priors = prior_tarsus)
...
setwd("/Users/augustarnstad/Library/CloudStorage/OneDrive-NTNU/Semester_10/R kladd/Animal model results")

saveRDS(model_mass, "model_mass_10.05.rds")
saveRDS(samples_mass, "samples_mass_10.05.rds")

saveRDS(model_wing, "model_wing_10.05.rds")
saveRDS(samples_wing, "samples_wing_10.05.rds")

saveRDS(model_tarsus, "model_tarsus_10.05.rds")
saveRDS(samples_tarsus, "samples_tarsus_10.05.rds")
```
